...
if(isset($_POST['activity'])) {
    if($_POST['activity'] == "add" && isset($_POST['hash'])) {
        ...
        $insert = insert("examqm","package='$hash', uniqueKey='$uK', question='$q', image='$i', a='$a', b='$b', c='$c', d='$d', e='$e', va='$v', value='$value'");
        if($insert) {$r['OK'] = true;}
        ...
        if (count($sign) == 2) {
            ...
            $get = select("examqm","package='$hash' AND ID='$ID'");
            if(mysqli_num_rows($get) == 1) {
                ...
                $v = json_encode($_POST['va']) ?? $data['va'];
                $value = $_POST['value'];
                ...
                $u = update("examqm","question='$q', image='$i', a='$a', b='$b', c='$c', d='$d', e='$e', va='$v', value='$value'","package='$hash' AND ID='$ID'");
                if($u) {$r['OK'] = true;}
            }
        }
        ...
        if (count($sign) == 2) {
            ...
            $u  = update("examqm","image=''","package='$hash' AND ID='$ID'");
            if($u) {$r['OK'] = true;}
        }
        ...
        if (count($sign) == 2) {
            ...
            $d = delete("examqm","package='$hash' AND ID='$ID'");
            if($d) {$r['OK'] = true;}
        }
    }
}
