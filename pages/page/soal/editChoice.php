    ...
    <form data-token="add">
        ...
        <div class="row" style="margin-top: 10px;">
            ...
            <div class="col-md-6 answers">
                <div class="choices">
                    <div class="input-group"><span class="input-group-addon">A</span><input type="text" name="choice_a" class="form-control" /></div>
                    <div class="input-group"><span class="input-group-addon">B</span><input type="text" name="choice_b" class="form-control" /></div>
                    <div class="input-group"><span class="input-group-addon">C</span><input type="text" name="choice_c" class="form-control" /></div>
                    <div class="input-group"><span class="input-group-addon">D</span><input type="text" name="choice_d" class="form-control" /></div>
                    <div class="input-group"><span class="input-group-addon">E</span><input type="text" name="choice_e" class="form-control" /></div>
                    <div style="display: grid;grid-template-columns: 2fr 1fr;gap: 15px;">
                        <div class="input-group">
                            <span class="input-group-addon">BENAR</span>
                            <select name="va" class="form-control">
    ...
    while ($data = mysqli_fetch_array($GET)) {
        ...
        <div class="pilgand" data-token="<?= $rtoken ?>">
            <form data-token="<?= $rtoken ?>" data-sign="<?= $hash . ":" . $data['ID'] ?>">
                <div class="row">
                    <div class="col-md-12 d-flex space-between" style="align-items: center;">
                        ...
                        <div class="pull-right">
                            ...
                            <button class="btn-upload btn btn-sm btn-info btn-fill" data-token="<?= $rtoken ?>" disabled><i class="fa fa-image"></i></button>
                ...
                <div class="row" style="margin-top: 10px;">
                    <div class="col-md-6">
                        <textarea class="form-control" rows="4" name="question" data-saved="<?= $data['question'] ?>" placeholder="Pertanyaan" disabled><?= base64_decode($data['question']) ?></textarea>
                    </div>
                    <div class="col-md-6 answers">
                        <div class="choices">
                            <div class="input-group"><span class="input-group-addon">A</span><input type="text" name="choice_a" class="form-control" data-saved="<?= base64_decode($data['a']) ?>" value="<?= base64_decode($data['a']) ?>" disabled /></div>
                            <div class="input-group"><span class="input-group-addon">B</span><input type="text" name="choice_b" class="form-control" data-saved="<?= base64_decode($data['b']) ?>" value="<?= base64_decode($data['b']) ?>" disabled /></div>
                            <div class="input-group"><span class="input-group-addon">C</span><input type="text" name="choice_c" class="form-control" data-saved="<?= base64_decode($data['c']) ?>" value="<?= base64_decode($data['c']) ?>" disabled /></div>
                            <div class="input-group"><span class="input-group-addon">D</span><input type="text" name="choice_d" class="form-control" data-saved="<?= base64_decode($data['d']) ?>" value="<?= base64_decode($data['d']) ?>" disabled /></div>
                            <div class="input-group"><span class="input-group-addon">E</span><input type="text" name="choice_e" class="form-control" data-saved="<?= base64_decode($data['e']) ?>" value="<?= base64_decode($data['e']) ?>" disabled /></div>
                            <div style="display: grid;grid-template-columns: 2fr 1fr;gap: 15px;">
                                <div class="input-group">
                                    <span class="input-group-addon">BENAR</span>
                                    <select name="va" class="form-control" data-saved="<?= $data['va'] ?>" disabled>
                                        <?Php $fc = ["a", "b", "c", "d", "e"];
                                        for ($i = 0; $i <= count($fc) - 1; $i++) { ?><option value="<?= $fc[$i] ?>" <?= $data['va'] == $fc[$i] ? "selected" : "" ?>><?= strtoupper($fc[$i]) ?></option><?Php } ?>
                                    </select>
                                </div>
                                <div>
                                    <div>
                                        <input type="number" name="value" class="form-control" value="<?= htmlspecialchars($data['value']) ?>" data-saved="<?= htmlspecialchars($data['value']) ?>" disabled>
                                    </div>
                                </div>
                            </div>
                        </div>
...
<script>
    ...
    $(".pilgand.add input[type='button']").on("click", function() {
        ...
        formData.append("action", "crud/ujian/csoal");
