<style>
    ...
    .option {margin-top: 16px;display: grid;gap: 15px;grid-template-columns: repeat(4, 1fr)}

    @media (min-width: 992px) {
        .option {margin-top: 0px;}
        .option {grid-template-columns: repeat(2, 1fr);}
    }
</style>
<div class="newEssays essays" data-token="add">
    ...
    <input type="text" class="form-control d-block mt-3" name="answer" placeholder="Jawaban" />
    <select name="newValue" class="form-control" style="margin-top: 15px;">
        <option value="0">- BOBOT JAWABAN BENAR -</option>
        <?php for($x = 1;$x <= 100;$x++) { echo "<option value='".$x."'>".$x."</option>"; } ?>
    </select>
    <button class="saveNew btn btn-success btn-fill mt-4">Simpan</button>
</div>
<?Php $no = 1; while($data = mysqli_fetch_array($GET)) { $rtoken = randomtoken(64); ?>
<div class="essays"  data-token="<?= $rtoken ?>" data-sign="<?= $hash.":".$data['ID'] ?>">
    ...
    <div class="row">
        <div class="col-md-10">
            ...
            <input type="text" class="form-control d-block mt-3" name="answer" placeholder="Jawaban" data-saved="<?= base64_decode($data['answer_text']) ?>" value="<?= base64_decode($data['answer_text']) ?>" disabled />
        </div>
        <div class="col-md-2">
...
<script>
    ...
    $(".saveNew").on("click", function(e){
        ...
        $.ajax({
            ...
            type: "POST",
            data: {"action":"crud/ujian/psoal","activity":"add","hash":hash,"question":question,"value":value,"answer":answer},
    ...
    $(".btn-save").on("click", function(e){
        ...
        var question = $(`.essays[data-token=${token}] textarea[name='question']`).val(),
            answer = $(`.essays[data-token=${token}] input[name='answer']`).val(),
            value = $(`.essays[data-token=${token}] select[name='value']`).val();;

        $.ajax({
            url: "<?= base_url("endpoint/index.php") ?>",
            type: "POST",
            data: {"action":"crud/ujian/psoal","activity":"update","sign":sign,"question":question, "answer":answer, "value":value},
            success:function(data){
                var response = JSON.parse(data);
                console.log(data);
                if(response.OK == true) {
                    ...
                    $(`.essays[data-token=${token}] textarea, .essays[data-token=${token}] button, .essays[data-token=${token}] select`).attr("disabled","disabled");
    ...
    $("button.btn-delete").on("click", function(e){
        ...
        if(confirm("Apakah Anda yakin ingin menghapus essai ini?")) {
            $.ajax({
                url: "<?= base_url("endpoint/index.php") ?>",
                type: "POST",
                data: {"action":"crud/ujian/psoal","activity":"delete","sign":sign},
                success:function(data){
                    var response = JSON.parse(data);
                    console.log(data);
                    if(response.OK == true) {
